...
class File {
  ...
  public static inline function getBytes(path:String):Bytes return asys.FileSystem.readFile(path);
  public static inline function getContent(path:String):String return asys.FileSystem.readFile(path).toString();
  //static inline function read(path:String, ?binary:Bool = true):FileInput return sys.FileSystem.open(path, Read, binary).input;
  public static inline function saveBytes(path:String, bytes:Bytes):Void return asys.FileSystem.writeFile(path, bytes);
  public static inline function saveContent(path:String, content:String):Void return asys.FileSystem.writeFile(path, Bytes.ofString(content));
